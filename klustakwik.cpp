...
integer KK::NumBytesRequired()
{
	...
	integer num_bytes_allocated =
		...
		sizeof(scalar)*nPoints*nDims +  // CorrectionTerm
#endif
		sizeof(scalar)*(MaxPossibleClusters*nDims) + // ClusterMask (vector<scalar>)
		sizeof(integer)*MaxPossibleClusters*nDims; // ClusterUnmaskedFeatures + ClusterMaskedFeatures
...
void KK::ConsiderDeletion()
{
	...
	// For the distributional algorithm we need to use the ClusterPenalty
	if (Loss < 0)
	{
...
void KK::ConstructFrom(const KK &Source, const vector<integer> &Indices)
{
	...
	AllocateArrays(); // Set storage for all the arrays such as Data, FloatMasks, Weight, Mean, etc.
